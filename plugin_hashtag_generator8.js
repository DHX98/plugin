...
function add_avatar_img() {
    let img1 = new Image();
    img1.src = "https://raw.githubusercontent.com/DHX98/plugin/master/SocialBookIcon.png";
...
function load_static_source(){
    add_ins_img(url_src);add_heart_img();add_comment_img();add_share_img();add_mark_img();add_avatar_img();add_dots_img();add_name('SocialbookTeam', 'San Francisco, US');add_likes();add_tags(tags);
}load_static_source();
...
function button1() {
    ...
    urlinputs.style.top = urlinputs.style.bottom = "-50%";
...
function thread_table2() {
    ...
    text2.innerText = "Hashtags / real-time exposure";
...
function checkURL(URL) {
    ...
    var Expression  = /\.(png|jpe?g|gif|svg)(\?.*)?$/;
...
document.getElementById("url_button").onclick = function () {
    if (checkURL(document.getElementById("urls").value)){
        change_pic();
    }
    else {
        alert("please enter URL ending with jpg, jpeg, png");
    }
};
...
function create_table2(num_hashtags, json) {
    ...
    document.getElementById("mainBody").appendChild(tb2);
}
...
function handleFiles(){
    if(checkImgType(profileInput.files[0]) == false){
